  ...
  <aside class="w-[270px] h-screen flex-shrink-0 border-r border-outline-variant bg-surface flex flex-col z-30 select-none shadow-sm">
    ...
    <div class="p-3 border-b border-outline-variant bg-surface-container-low/40">
      ...
      >
        <div class="flex items-center gap-2.5 overflow-hidden">
          ...
          ></span>
          <div class="truncate">
            <div class="text-xs font-bold text-on-surface truncate group-hover:text-primary transition-colors">{{ store.activeConnection.name }}</div>
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-3">
      <div v-if="store.activeConnection">
        ...
        <!-- Tables Section -->
        <div>
          <div class="flex items-center justify-between text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-2 px-1 font-mono">
            <span>Tables ({{ filteredTables.length }})</span>
            <button @click="store.refreshExplorer()" class="text-on-surface-variant hover:text-primary transition-colors" title="Refresh">
              <span class="material-symbols-outlined text-sm">refresh</span>
            </button>
          </div>

          <div v-if="filteredTables.length === 0" class="text-xs text-on-surface-variant px-1 italic font-mono">
            No tables found
          </div>
          ...
          <div v-else class="space-y-0.5">
            <button
              ...
              class="w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-xs transition-colors text-left group"
              :class="store.selectedTable === t.name ? 'bg-surface-container-high text-primary font-bold border border-primary/30 shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface'"
            >
              <div class="flex items-center gap-2 truncate">
                <span class="material-symbols-outlined text-[16px] text-on-surface-variant group-hover:text-primary transition-colors">table_rows</span>
                <span class="truncate font-mono">{{ t.name }}</span>
              </div>
              <span v-if="t.rowCount" class="text-[10px] text-on-surface-variant font-mono bg-surface-container-low px-1.5 py-0.2 rounded border border-outline-variant/40">{{ t.rowCount }}</span>
            </button>
          </div>
        </div>
        ...
        <div v-if="filteredViews.length > 0" class="pt-2">
          <div class="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-2 px-1 font-mono">
          ...
          <div class="space-y-0.5">
            <button
              ...
              class="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs transition-colors text-left group"
              :class="store.selectedTable === v.name ? 'bg-surface-container-high text-primary font-bold border border-primary/30 shadow-sm' : 'text-on-surface-variant hover:bg-surface-container-low hover:text-on-surface'"
            >
              <span class="material-symbols-outlined text-[16px] text-on-surface-variant group-hover:text-primary transition-colors">visibility</span>
